...
async function getCollectionUserList(): Promise<UserResponse[]> {
  ...
  const q = query(userCollection, where("useYn", "==", "N")); // useYn == "N" 조건 추가
...
export default async function OrgRightTabServer() {
  const session = await getServerSession(authOptions);
  const userList = await getCollectionUserList();

  return <OrgRightTabClient session={session!} userList={userList} />;
}
