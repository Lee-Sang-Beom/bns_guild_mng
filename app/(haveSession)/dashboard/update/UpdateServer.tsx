...
const UpdateClient = dynamic(() => import("./UpdateClient"), {
  ssr: false, // 서버 사이드 렌더링 비활성화
});

async function getCollectionupdate(
  queryInstance: UpdateRequest
): Promise<TablePageResponse<UpdateResponse[]>> {
  let {
    page,
    size,
    sort,
  ...
  const updateCollection = collection(db, "collection_update");
  ...
  // 총 문서 수
  const filteredQuery = query(updateCollection, ...queryConstraints);
  ...
  // Firestore 쿼리 실행
  const q = query(updateCollection, ...queryConstraints);
  const querySnapshot = await getDocs(q);

  // 결과를 UpdateResponse 형식으로 변환
  const update: UpdateResponse[] = querySnapshot.docs.map(
    (doc) =>
      ({
        docId: doc.id,
        ...doc.data(),
...
export default async function UpdateServer({
  searchParams,
}: {
  ...
  let queryInstance: UpdateRequest = {
    page: 1,
    size: 5,
    sort: "regDt",
    orderBy: "desc",
  ...
  const tableResponse: TablePageResponse<UpdateResponse[]> =
    await getCollectionupdate(queryInstance);
