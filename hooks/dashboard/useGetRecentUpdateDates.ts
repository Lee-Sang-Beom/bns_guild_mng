...
const fetchRecentUpdateDates = async (): Promise<NoticeResponse[]> => {
  const noticeCollection = collection(db, "collection_update");
...
// React Query 훅
export const useGetRecentUpdateDates = () => {
  return useQuery({
    queryKey: ["useGetRecentUpdateDates"],
    queryFn: fetchRecentUpdateDates,
    initialData: [],
  });
};
